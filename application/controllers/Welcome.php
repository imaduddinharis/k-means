...
class Welcome extends CI_Controller {
	...
	public function index()
	{
		...
		$getCentroidC2 = Province::where('id','110')->get();
		...
		foreach($getAllData as $datas=>$values):
			...
			$opt1 = pow(($d1-$c1d1),2);
			$opt1 = $opt1 + pow(($d1-$c2d1),2);
			$opt1 = $opt1 + pow(($d1-$c3d1),2);
			$opt1 = sqrt($opt1);

			$opt2 = pow(($d2-$c1d2),2);
			$opt2 = $opt2 + pow(($d2-$c2d2),2);
			$opt2 = $opt2 + pow(($d2-$c3d2),2);
			$opt2 = sqrt($opt2);

			$opt3 = pow(($d3-$c1d3),2);
			$opt3 = $opt3 + pow(($d3-$c2d3),2);
			...
			foreach($arrDataC1Iterasi as $valC1x){
				...
				$c3d3x = $c3d3x / count($arrDataC3Iterasi);
					...
					foreach($getAllData as $datas=>$values):
						...
						$opt1 = pow(($d1-$c1d1x),2);
						$opt1 = $opt1 + pow(($d1-$c2d1x),2);
						$opt1 = $opt1 + pow(($d1-$c3d1x),2);
						$opt1 = sqrt($opt1);
			
						$opt2 = pow(($d2-$c1d2x),2);
						$opt2 = $opt2 + pow(($d2-$c2d2x),2);
						$opt2 = $opt2 + pow(($d2-$c3d2x),2);
						$opt2 = sqrt($opt2);
			
						$opt3 = pow(($d3-$c1d3x),2);
						$opt3 = $opt3 + pow(($d3-$c2d3x),2);
		...
		// return false;
			...
			if($validationCluster != 0){
				...
				endforeach;
				foreach($idC2 as $valItC1):
					if(in_array($valItC1,$idC2tmp)){
						// $validationCluster = $validationCluster + 1;
					}else{
						$validationCluster = $validationCluster + 1;
					}
			...
			if($validationCluster != 0){
				...
				endforeach;
				foreach($idC3 as $valItC1):
					if(in_array($valItC1,$idC3tmp)){
						// $validationCluster = $validationCluster + 1;
					}else{
						$validationCluster = $validationCluster + 1;
					}
